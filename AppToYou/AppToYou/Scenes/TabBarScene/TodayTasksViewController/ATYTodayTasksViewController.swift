...
//                let firstDate = task.date?.ignoringTime
//                let secondDate =
//                calendar.compare(<#T##date1: Date##Date#>, to: <#T##Date#>, toGranularity: <#T##Calendar.Component#>)
//                calendar.compare( $0.date?.ignoringTime ?? Date(), to: Date().ignoringTime ?? Date(), toGranularity: .day) == .orderedSame
//            }
//            .filter( { Calendar.current.compare( $0.date?.ignoringTime ?? Date(), to: Date().ignoringTime ?? Date(), toGranularity: .day) == .orderedSame } )
